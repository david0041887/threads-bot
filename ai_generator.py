...
【骨架 E：引流型（資訊缺口）】
目標：讓讀者產生「這個我想問清楚」的念頭，不是給完整答案

結構：
1. 開場：生活觀察 / 具體情境 / 疑問——不可以知識開場，要先有畫面
2. 中段：帶出一個「重要但不講完」的觀念，點到為止
3. 轉折：點出多數人忽略的地方，但刻意留缺口
4. 結尾：隱性 CTA，讓人覺得可以追問，但沒有壓力

開場變化（每次隨機選一種）：
- 抱怨型：「最近發現一件有點奇怪的事…」「每次聊到這個我都覺得很可惜」
- 對話型：「前幾天有人問我…」「上週碰到一個狀況」
- 自我觀察：「我以前也以為這樣就夠了」「做這行久了，有些東西會開始懂」
- 客戶案例：「有個人遇到這個問題，結果很出乎意料」

隱性 CTA（結尾隨機選一類，語感自然不強迫）：
...
效果自查（輸出前必做）：
- 是否看起來像在賣東西？→ 是的話重寫
- 是否把知識講太完整？→ 刪掉 20% 最關鍵的那段
- 是否讓人產生想問的疑問？→ 沒有的話重寫結尾
...
規則：
- **結構強制**：3 篇草稿中——第 1 篇用骨架 E（引流型），第 2、3 篇從 A/B/C/D 選不重複的骨架
- **排版強制**：每篇草稿一定用空行（\\n\\n）分隔 stanza，不得單段長文；每個 stanza 1~3 行
- 每篇草稿不超過 400 字（含標點，空行不計）；骨架 E 控制在 100～200 字
- 絕對不加任何 hashtag 或 # 字號
- 不直接推銷保險商品
- 骨架 D 客戶案例：年齡、情境要具體，不能說出具體保費數字；重點在「做了什麼 → 解決了什麼」
- 骨架 E 引流型：嚴格依照引流型規則，刻意留缺口，結尾用隱性 CTA，不可把知識講完整
- 語氣嚴格比照 persona「語氣校準」與各骨架範例，避免 AI 公文腔
- 輸出嚴格為 JSON array，不加任何說明文字，不加 markdown 符號
