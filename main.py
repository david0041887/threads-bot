...
SEARCH_KEYWORDS = [
    ...
    # 想轉換保經 / 換業務員（高意向 lead：客戶想換承辦、或同業想跳槽保經）
    "孤兒保單", "換業務員", "換保經", "保經推薦", "推薦保經",
...
# 目前取 1 小時（一天 24 次）：David 要的是「發文一小時內就推播」，而貼文最壞情況會等滿
# 一個間隔才被掃到，所以間隔本身就是新鮮度的硬上限，2 小時做不到這個要求。
# 一天 24 次仍是殺掉 session 那次（144 次）的六分之一。若這份 cookie 又在一兩天內死掉，
# 代表 24 次/天 仍踩線，下一步不是再往下調頻率，而是把海巡搬到本機筆電跑
# （住宅 IP 與 cookie 來源一致，頻率就不再是限制）。
